...
- The board is used in an admin-only internal context, so authentication and authorisation are out of scope.
- Upstream source data is assumed to be valid and consistent enough for display without adding a client-side normalisation layer.
...
- Centralised frontend API requests in a service module (`client/src/services/api.js`) to isolate network concerns from components.
...
- Add authentication/authorisation so status changes are restricted to appropriate admin roles.
...
- Containerise and add CI steps (lint/test/build) for consistent local/dev/prod workflows.
